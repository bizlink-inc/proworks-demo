...
import { createRecommendationClient, createJobClient, getAppIds } from "@/lib/kintone/client";
...
export const GET = async () => {
  try {
    ...
    // 1. ユーザーに対する全ての推薦レコードを取得
    const condition = `${RECOMMENDATION_FIELDS.TALENT_ID} = "${authUserId}"`;
    ...
    // 3. 関連する案件情報を取得
    const jobIds = [...new Set(notificationTargets.map((rec) => rec[RECOMMENDATION_FIELDS.JOB_ID]?.value as string))];
    ...
    for (const jobId of jobIds) {
      ...
      try {
        ...
        jobsMap.set(jobId, {
          ...
          createdAt: jobRecord.作成日時?.value || new Date().toISOString(),
        });
      } catch (error) {
        console.error(`案件取得エラー (jobId: ${jobId}):`, error);
      }
    }
