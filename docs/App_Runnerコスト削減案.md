...
- 開発環境を 24 時間稼働 → **使用時のみ起動**

**料金比較（0.25 vCPU、0.5 GB の場合）**:

| 起動パターン            | 起動時間/月 | 料金計算               | 月額料金                 |
| ----------------------- | ----------- | ---------------------- | ------------------------ |
| 24 時間稼働             | 730 時間    | 0.02475 USD/時間 × 730 | 約 18 USD（約 2,700 円） |
| 平日 9~20 時（11 時間） | 220 時間    | 0.02475 USD/時間 × 220 | 約 5.4 USD（約 800 円）  |
| 1 日 8 時間             | 240 時間    | 0.02475 USD/時間 × 240 | 約 6 USD（約 900 円）    |

**削減効果（平日 9~20 時の場合）**:

- **約 12.6 USD/月（約 1,900 円/月）の削減**
- 24 時間稼働（18 USD）→ 平日 9~20 時（5.4 USD）

**注意点**:

- コールドスタートが発生する可能性（初回アクセス時に遅延）
- 使用時のみ手動で起動する必要がある
- 平日 9~20 時パターンは営業時間のみ稼働するため、最もコスト効率が良い
...
- 開発環境: 0.25 vCPU、0.5 GB、平日 9~20 時稼働（または停止可能）

**料金計算**:

- **本番環境**: 0.02475 USD/時間 × 730 時間 = 約 18 USD/月
- **開発環境**: 0.02475 USD/時間 × 220 時間（平日 9~20 時） = 約 5.4 USD/月
- **自動デプロイ**: 2 USD/月
- **合計**: 約 25.4 USD/月（約 3,800 円/月）

**削減効果**:

- **約 147.6 USD/月（約 22,200 円/月）の削減**
...
3. **最終的な目標**
   ...
   - 開発: 0.25 vCPU、0.5 GB、平日 9~20 時稼働 → 約 5.4 USD/月（約 800 円/月）
   - 合計: 約 25.4 USD/月（約 3,800 円/月）
...
| 削減案 2（平日 9~20 時） | 約 92.4 USD（約 13,900 円） | 約 80.6 USD  | コールドスタート（開発環境のみ平日稼働、0.25 vCPU、0.5 GB） |
| 削減案 3（組み合わせ）   | 約 25.4 USD（約 3,800 円）  | 約 147.6 USD | パフォーマンス + コールドスタート                           |
